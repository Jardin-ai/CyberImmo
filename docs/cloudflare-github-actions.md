...
#### 排错：浏览器 **1101 Worker threw exception**

多为 **运行时** `process.env.NEXT_PUBLIC_*` 在 Worker 里为空（例如 `createServerClient(undefined, …)` 抛错）。构建阶段有 Secret 不等于 Worker 进程里一定有同名变量。

**做法（二选一）**：

1. **推荐**：使用当前 [`deploy.yml`](../.github/workflows/deploy.yml)已在 `wrangler deploy` 上加入的 `--var NEXT_PUBLIC_SUPABASE_URL` / `ANON_KEY` / `NEXT_PUBLIC_BASE_PATH`与 `--keep-vars`。
2. 在 Cloudflare 控制台 →该 Worker → **Settings → Variables** 手动添加同名变量；之后部署务必带 **`--keep-vars`**，否则可能被清空。

仍1101 时：控制台 → **Workers → cyberimmo → Logs**，查看实时堆栈。

#### 排错：`createRequire` / `Received 'undefined'`（code 10021，`middleware/handler.mjs`）

**@opennextjs/cloudflare 1.5.x** 对 Next 16 的 **`proxy.ts`** 支持不完整，上传 Worker 时可能在中间件包里触发 `createRequire(undefined)`。

**本仓库做法**：会话与路由保护逻辑放在 **`src/middleware.ts`**，导出 **`middleware`**（与旧版 Next 一致）。构建时可能有一条「请改用 proxy」的弃用提示，可暂忽略；日后升级 **`@opennextjs/cloudflare`** 到官方已支持 Next 16 `proxy` 的版本后，可再迁回 `proxy.ts`。

#### 排错：`CPU limits are not supported for the Free plan`（code 100328）

[`web/wrangler.toml`](../web/wrangler.toml) 里 **不要** 在免费账号下保留 `[limits]` / `cpu_ms`。删掉后重新 `wrangler deploy`。付费 Workers 可按需再加。
...
### Cloudflare Worker（仪表盘）变量与 Observability

部署成功后，**Worker → Settings → Variables** 里与当前应用相关的约定如下。

| 类型 | 名称 | 是否必需 | 说明 |
|------|------|----------|------|
| 变量 | `NEXT_PUBLIC_BASE_PATH` | **建议与构建一致** | 子路径部署填 `/cyberimmo`；与 CI /本地 `NEXT_PUBLIC_BASE_PATH` 不一致会导致资源路径错乱。 |
| 变量 | `NEXT_PUBLIC_SUPABASE_URL` | **必需** | Supabase 项目 URL；middleware 与客户端共用。 |
| 变量 | `NEXT_PUBLIC_SUPABASE_ANON_KEY` | **必需** | Supabase anon key（配合 RLS）；勿把 **service_role** 填进 `NEXT_PUBLIC_*`。 |
| Secret | `ZHIPU_API_KEY` | 可选 | 仅 [`/api/chat`](../web/src/app/api/chat/route.ts) 需要智谱时配置。 |
| Secret | `DASHSCOPE_API_KEY` | 可选 | 仅 [`/api/persona/clone`](../web/src/app/api/persona/clone/route.ts) 需要通义时配置。 |
| Secret | `SUPABASE_SERVICE_ROLE_KEY` | 可选 | 仅服务端管理员逻辑（[`admin` Supabase 客户端](../web/src/lib/supabase/admin.ts)）需要；**务必用 Secret**。 |
| Secret | `ADMIN_SECRET` | 可选 | 仅 [`admin` 相关 Server Actions](../web/src/app/admin/actions.ts) 校验时使用。 |

**与 CI 的关系**：workflow 里若对同一 Worker 使用 `wrangler deploy --var ...` 且带 `--keep-vars`，则 **仪表盘里多出来的 Secret/变量会保留**；仅「与本地 `wrangler.toml` /命令行冲突的字段」会被覆盖。**Observability**（Logs / Traces）以仓库内 [`web/wrangler.toml`](../web/wrangler.toml) 为准，建议在文件中写明 `[observability.logs]`（及可选的 `[observability.traces]`），避免每次部署把控制台已开的日志关掉。

- **Logs**：`console`输出、未捕获异常栈，排1101、路由错误最直接。  
- **Traces**：单次请求内外部 `fetch`、绑定调用的耗时分解；流量大时可在 `wrangler.toml` 里把 `head_sampling_rate` 调到 `0.05`～`0.2`。  
  官方说明：[Workers Logs](https://developers.cloudflare.com/workers/observability/logs/workers-logs/)、[Traces](https://developers.cloudflare.com/workers/observability/traces/)。
