...
- **最常见 A — 只上传了静态目录，没有 OpenNext Worker**  
  OpenNext 的路由与 SSR 在 **`.open-next/worker.js`**。若仅用 `wrangler pages deploy .open-next/assets` 且目录里 **没有** **`_worker.js`**，Pages 只会按「纯静态」找文件，Next 的动态路由几乎全部 **404**。  
  **修复**（已写入 [`deploy.yml`](../.github/workflows/deploy.yml)）：构建后执行 `cp .open-next/worker.js .open-next/assets/_worker.js`，再 `pages deploy`（Pages [高级模式](https://developers.cloudflare.com/pages/functions/advanced-mode/) 从输出目录根读取 `_worker.js`）。官方也推荐用 `opennextjs-cloudflare deploy`（走 `wrangler deploy` + `main`/`assets` 配置），与「只 deploy assets」不同。
